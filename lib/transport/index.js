...
function Transport(context) {
  ...
  this._options = {
    silent: context.options.silent || false,
    failsafe: context.options.failsafe || false,
    send: context.options.send || null, // 添加第三方方法 如： socket.send(data)
  };
  this._execWith = '';
...
Transport.prototype.log = function(message) {
  ...
  // 使用 socket.io 实时传给前端
  if (this._options.send && typeof this._options.send === 'function') {
