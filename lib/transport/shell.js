...
Shell.prototype._exec = function(command, options) {
  ...
  // console.log('shell.js options: ', self._options, options);
  ...
  proc.on('close', function(code) {
    ...
    if(result.code === 0) {
      ...
      // 使用 socket.io 实时传给前端
      if (options.send && typeof options.send === 'function') {
        options.send('Command failed (' + result.code + '), please retry it.\n');
      }
    } else {
      ...
      self._logger.error('failed (' + error + ')');
      // 使用 socket.io 实时传给前端
      if (options.send && typeof options.send === 'function') {
        options.send('failed (' + error + ')\n');
      }
